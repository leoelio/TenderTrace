...
class EscalationDeliveryResult:
    ...
    relationship_action_count: int = 0
    message_id: str = ""
    reason: str = ""
...
def send_opportunity_escalation_summary(
    ...
    *,
    ...
    decision_count, task_count, change_review_count, relationship_action_count = (
        _issue_counts(items)
    )
    if not items:
        return EscalationDeliveryResult(
            ...
            artifact_key="",
            reason=(
                "no overdue opportunity decisions, tasks, relationship actions, "
                "or change reviews"
            ),
        )
    ...
    if not force and _already_sent(settings, artifact_key):
        return EscalationDeliveryResult(
            ...
            relationship_action_count=relationship_action_count,
...
def build_escalation_card(
    ...
    for index, raw in enumerate(escalations[:8], start=1):
        ...
        deadline_text = _deadline_text(item, issue_types)
        rows.append(
            {
                "tag": "div",
                "text": {
        ...
        "elements": [
            {
                ...
                "text": {
                    ...
                    "content": (
                        ...
                        f"任务逾期 {task_count} 条 · "
                        f"变更复核逾期 {change_review_count} 条 · "
                        f"关系行动逾期 {relationship_action_count} 条 · "
...
def _issue_types(item: dict[str, Any]) -> tuple[str, ...]:
    ...
    if isinstance(raw, list):
        ...
        values = tuple(
            value
            for value in ("decision", "task", "change_review", "relationship_action")
            if value in present
        )
        if values:
            return values
...
def _issue_counts(escalations: list[object]) -> tuple[int, int, int, int]:
    ...
    return (
        ...
        sum("change_review" in values for values in issue_sets),
        sum("relationship_action" in values for values in issue_sets),
    )
...
def _issue_label(issue_types: tuple[str, ...]) -> str:
    labels = {
        "decision": "决策超时",
        "task": "任务逾期",
        "change_review": "变更复核逾期",
        "relationship_action": "关系行动逾期",
    }
    return " + ".join(labels[value] for value in issue_types)


def _deadline_text(item: dict[str, Any], issue_types: tuple[str, ...]) -> str:
    labels = {
        "decision": ("决策截止", "decision_due_at"),
        "task": ("任务截止", "task_due_at"),
        "change_review": ("复核截止", "change_review_due_at"),
        "relationship_action": ("关系行动截止", "relationship_action_due_at"),
    }
    return " · ".join(
        f"{labels[value][0]} {_text(item.get(labels[value][1]) or item.get('due_at'), '-')}"
