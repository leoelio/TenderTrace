...
| 模型与评测 | 支持纯规则、本地 Ollama 和 OpenAI 兼容云端增强，运行时可切换。生成摘要先读取公告中金额、日期、项目编号等带原文位置的事实锚点；数字或文本支撑度校验不通过即回退抽取式摘要，低可信首轮结果最多触发一次扩域补采。评测覆盖意图 Harness、RAG 证据、Agent checkpoint/trace、来源可靠性、Recall Proxy 和人工金标 Recall@K；代理指标不冒充严格召回率。业务价值面板只汇总质量复核通过的同类任务基线/辅助耗时，无实测数据时不估算节省。 |
