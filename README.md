...
<p align="center">
  <sub>Current stage: P53 · 16 sources · multi-agent review, executable Feishu war room & fact-gated LLM summary · automated regression suite</sub>
</p>
...
| 决策与执行 | 统一动作契约控制认领、事实复核、Go/Hold/No-Go、投标准备、结果和归档。团队覆盖、客户关系、证据完整度、机会评分和投标窗口共同构成可解释门禁；重大公告变更会使旧决策失效并进入复核与 SLA 升级。会审 case 的幂等键包含公告修订版本：同版重复同步不重复建项，新版变化会开启新的复核轮次。Agent 建议、网页与群内人工意见以及人工裁决均保留原文证据，不覆盖要求账本；协作意见只能由确认人显式转为行动，未指定负责人或期限时不会自动补全，满足条件后也仅同步该条确认要求至飞书 Task v2。 |
...
| 模型与评测 | 支持纯规则、本地 Ollama 和 OpenAI 兼容云端增强，运行时可切换。生成摘要先读取公告中金额、日期、项目编号等带原文位置的事实锚点；数字或文本支撑度校验不通过即回退抽取式摘要，低可信首轮结果最多触发一次扩域补采。会审 Agent 将公告和企业材料视为不可信引用：疑似指令注入会阻断模型裁决、保留原因并升级人工核验。评测覆盖意图 Harness、RAG 证据、Agent checkpoint/trace、来源可靠性、Recall Proxy 和人工金标 Recall@K；代理指标不冒充严格召回率。业务价值面板只汇总质量复核通过的同类任务基线/辅助耗时，无实测数据时不估算节省。 |
...
| 战情室与会审 | 机会详情先给出卡片、负责人任务、截止日历、多维表格、五角色会审与要求任务的启动前检查；已选项目群会成为预检和启动的同一接收目标，确认后会先按要求账本和公告变更生成会审 case，再执行飞书资源创建并将逐步结果写入机会审计事件。五个独立 Agent 分别从项目统筹、合规、技术、商务和证据审计角度形成带分歧标记的建议，人工裁决仍是唯一可改变会审状态的动作；会审摘要卡可按当前账本状态同步到项目群并去重。群成员可发送 `项目意见 <机会编号>：<内容>` 记录机会协作判断，或发送 `会审意见 <机会编号> <要求编号>：<内容>` 为指定要求补充正式会审依据；两类信息均带作者、渠道、时间并回写审计链。意见被确认后可形成可完成、可追溯的行动；只有负责人和截止时间均由人工补齐时，用户才可显式把对应的单条要求同步为飞书任务。 |
...
| 页面 | 主要用途 |
|---|---|
| 工作台 | 输入查询、预览意图、选择立即运行或创建订阅；飞书交付可直接指定已登记项目群，后续群内成员可读取报告、发表项目意见或要求级会审意见。 |
...
| Decision and execution | A shared action contract governs claiming, review, Go/Hold/No-Go, bid preparation, outcome, and archive actions. Material notice changes invalidate stale decisions and enter an SLA-bound review workflow. Review-case idempotency includes the notice revision: duplicate delivery of one revision creates no duplicate case, while a later material revision starts a new review round. A collaboration opinion becomes an action only after an explicit human confirmation; absent owners or deadlines are never invented, and a qualified action can sync only its matching requirement to Feishu Task v2. |
...
| Models and evaluation | Supports rule-only, local Ollama, and OpenAI-compatible cloud modes. Generated summaries receive source-positioned amount, date, and project-number anchors first; unsupported numbers or weakly grounded text fall back to extractive output, and a low-trust first batch can trigger one bounded broadened recollection. Review agents treat tender and capability material as untrusted quotations: suspected instruction injection blocks model adjudication, preserves the reason, and escalates to a human. Evaluation covers intent harnesses, RAG evidence, Agent traces/checkpoints, source reliability, recall proxy, and manually annotated Recall@K. Business-value metrics only aggregate quality-reviewed task timings; they do not infer savings from clicks or unreviewed samples. |
...
| War room and review | The opportunity dossier preflights cards, ownership tasks, deadline calendar, Bitable, five-role review, and requirement tasks against the selected project group. Launch first derives review cases from the evidence ledger and material notice changes, then writes every result to the opportunity audit. Five independent Agents cover project control, compliance, technical, commercial, and evidence-audit perspectives; they produce evidence-based, disagreement-aware suggestions only, while an explicit human decision remains the sole state-changing action. The current evidence-ledger state can be sent as a deduplicated review digest card to that group. Group members can send `项目意见 <opportunity ID>：<content>` for a general collaboration note, or `会审意见 <opportunity ID> <requirement key>：<content>` to attach a human review opinion to the matching requirement; both preserve author, channel, time, and an audit event. A confirmed opinion can become a completable, auditable action; only a human-supplied owner and deadline unlock an explicit, single-requirement Feishu task sync. |
